...
export default function AdDashboard({ isOpen, onClose }: AdDashboardProps) {
  // Initializing view based on current user session
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.user) {
        setUser(result.user);
        setView('dashboard'); // Explicitly set view to ensure it moves forward
      }
    } catch (error) {
      console.error('Login Error:', error);
      alert('Login failed. Please try again. / लॉगिन विफल रहा। कृपया पुनः प्रयास करें।');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-8 no-scrollbar">
          {isAuthChecking ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
              ...
              <p className="text-slate-500 font-bold">Checking access... / पहुंच की जांच हो रही है...</p>
            </div>
          ) : (
