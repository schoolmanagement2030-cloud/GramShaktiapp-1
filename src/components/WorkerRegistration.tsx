      ...
      <AnimatePresence mode="wait">
        {isSuccess ? (
          ...
          <motion.form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input className="w-full p-3 border rounded-xl" placeholder="Mobile"
              value={formData.mobile}
              onChange={e => setFormData({ ...formData, mobile: e.target.value })}
            />

            <input className="w-full p-3 border rounded-xl" placeholder="Pincode" maxLength={6}
              value={formData.pincode}
              onChange={e => setFormData({ ...formData, pincode: e.target.value })}
            />

            <div className="relative">
              <button type="button" onClick={() => setIsVillageDropdownOpen(!isVillageDropdownOpen)}
              ...
              {isVillageDropdownOpen && (
                <div className="absolute w-full bg-white border mt-1 rounded-xl max-h-40 overflow-y-auto">
