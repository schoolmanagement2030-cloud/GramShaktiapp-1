...
import { motion, AnimatePresence } from 'framer-motion'; // ✅ FIXED: motion/react से framer-motion किया
...
export default function WorkerRegistration() {
  ...
  const dropdownRef = useRef<HTMLDivElement>(null); // ✅ FIXED: Type added
  ...
  const [formData, setFormData] = useState({
    ...
    pincode: '',
    village: '',
    category: `${MAIN_CATEGORIES[0].hindi} (${MAIN_CATEGORIES[0].english})` as Category, // ✅ FIXED: Template literal fixed
    subCategory: '',
    skills: '',
  ...
  const subDropdownRef = useRef<HTMLDivElement>(null); // ✅ FIXED: Type added
  ...
  useEffect(() => {
    if (selectedMainCat && selectedMainCat.subCategories && selectedMainCat.subCategories.length > 0) {
    ...
    return (
      <div className="bg-white p-8 rounded-2xl shadow-md border border-slate-100 text-center">
        ...
        >
          ...
          Sign in with Google / Google से साइन in करें
        </button>
      </div>
    );
  }
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input required type="text" placeholder="Enter your name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              </div>
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                  <Phone size={16} /> Mobile Number (मोबाइल नंबर)
                </label>
                <input required type="tel" pattern="[0-9]{10}" placeholder="10-digit number" value={formData.mobile} onChange={(e) => setFormData({ ...formData, mobile: e.target.value })} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              </div>
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                  <Home size={16} /> Pincode (पिनकोड)
                </label>
                <input required type="text" placeholder="6-digit pincode" value={formData.pincode} onChange={(e) => setFormData({ ...formData, pincode: e.target.value })} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              </div>
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                  <MapPin size={16} /> Village / Locality (गाँव / इलाका)
                </label>
                <input required type="text" placeholder="Enter village name" value={formData.village} onChange={(e) => setFormData({ ...formData, village: e.target.value })} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              </div>

              {/* Custom Category Dropdown */}
              <div className="space-y-2 relative" ref={dropdownRef}>
                <label className="block text-sm font-semibold text-slate-700">Main Category / मुख्य श्रेणी</label>
                <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none flex items-center justify-between gap-3 text-left" >
                ...
                {isDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 max-h-[300px] overflow-y-auto py-2">
                    {MAIN_CATEGORIES.map((cat) => {
                      ...
                      return (
                        <button key={cat.id} type="button" onClick={() => { setFormData({ ...formData, category: `${cat.hindi} (${cat.english})` as Category }); setIsDropdownOpen(false); }} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-emerald-50 transition-colors text-left group" >
              ...
              <div className="space-y-2 relative" ref={subDropdownRef}>
                ...
                <button type="button" onClick={() => setIsSubDropdownOpen(!isSubDropdownOpen)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none flex items-center justify-between gap-3 text-left" >
                  <span className="text-sm font-bold text-slate-700">
                ...
                {isSubDropdownOpen && selectedMainCat && selectedMainCat.subCategories && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 max-h-[300px] overflow-y-auto py-2">
                    {selectedMainCat.subCategories.map((sub, idx) => (
                      <button key={idx} type="button" onClick={() => { setFormData({ ...formData, subCategory: `${sub.hindi} (${sub.english})` }); setIsSubDropdownOpen(false); }} className="w-full px-4 py-2.5 hover:bg-emerald-50 transition-colors text-left" >
              ...
              <div className="space-y-2">
                ...
                <input required type="text" placeholder="e.g. Electrician, Grocery, Plumber" value={formData.skills} onChange={(e) => setFormData({ ...formData, skills: e.target.value })} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-6 rounded-2xl border border-dashed border-slate-300">
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <button type="button" onClick={captureLocation} disabled={isLoading} className={`flex items-center gap-2 py-3 px-6 rounded-xl font-bold transition-all ${formData.location ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-900 text-white hover:bg-slate-800'}`} >
                  <MapPin size={18} />
                  {formData.location ? 'Location Captured / स्थान कैप्चर किया गया' : 'Capture GPS / GPS कैप्चर करें'}
                </button>
              </div>
              {formData.location && (
                <p className="text-xs text-emerald-600 mt-2 font-medium"> ✓ Lat: {formData.location.lat.toFixed(4)}, Lng: {formData.location.lng.toFixed(4)} </p>
              )}
            </div>

            <button type="submit" disabled={isLoading} className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-2 transition-all transform active:scale-[0.98] shadow-lg shadow-emerald-200" >
              {isLoading ? (
